# --------------------------------------------------
# 國中資訊三年級期中考_python
# Streamlit 線上測驗系統（最終版）
#
# 功能：
# 1. 單選題 20 題
# 2. 학생輸入：班級、座號、姓名
# 3. 同一「班級＋座號」最多作答 2 次
# 4. 自動輸出 Excel（results.xlsx）
# 5. 老師專用下載 Excel（密碼：yjf007）
# --------------------------------------------------
...
# ===== 老師專用：下載 Excel（密碼保護）=====
st.divider()
st.subheader("老師專用功能（需輸入密碼）")

input_password = st.text_input("請輸入老師下載密碼", type="password")
        ...
        if os.path.exists(RESULT_FILE):
            with open(RESULT_FILE, "rb") as f:
                st.download_button(
                    label="📥 下載學生作答 Excel",
                    data=f,
                    file_name="學生作答成績.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )
        else:
            st.info("目前尚無學生作答資料。")
    else:
        st.error("❌ 密碼錯誤，無法下載成績。")